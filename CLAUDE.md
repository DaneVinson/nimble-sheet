...
- **In progress (2026-06-18, uncommitted — pending in-browser verification)**: a `TilePopover` fix. The inline-editor/play-tile popovers dismissed on a `svelte:window` **click** outside the popover; a native `<select>`'s option-selection emits a fall-through click outside the small popover, closing it before the user can press "Add" — so every inline editor's add-flow (reported via **Conditions**) silently failed. Fix: dismiss on outside **`pointerdown`** instead of `click` (native option selection dispatches no page pointerdown). Type-checks and builds clean and is deployed to `wwwroot`; awaiting a real-browser confirmation (the bug can't be reproduced by automation — Playwright sets `<select>` values programmatically, which is why it passed earlier checks). If confirmed, commit `NS.Client/src/lib/sheet/components/TilePopover.svelte`.

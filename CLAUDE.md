...
- **Shipped (2026-06-19)**: fixed the no-body play-mutations failing with **HTTP 415** (reported via **Wounds → Gain**; also affected heal-wound and recover-all-resources). FastEndpoints rejects DTO-bound `POST`/`PUT`/`PATCH` requests that lack `Content-Type: application/json` with **415, before the handler runs** — even when the DTO binds only route params and needs no body (e.g. `GainWoundEndpoint : Endpoint<HeroIdRequest>`, `HeroId` from the route). The client's `apiFetch` only set the content type when a body was present, so the no-body wrappers sent a bare `POST` and got 415; body-bearing mutations (take-damage, add-condition, …) always worked because they set it. Fix in `NS.Client/src/lib/api/client.ts`: `apiFetch` now always sends `Content-Type: application/json` for any mutating method, defaulting to a `{}` body when there's no payload. Confirmed at the wire with curl (415 without the header → 404 with it on a bogus id, i.e. the handler is reached) and browser-verified end-to-end (Gain → 204, wound pip 0→1). 63 client unit tests pass. **Gotcha to remember:** any new no-body mutation endpoint is fine as long as calls go through `apiFetch`.
